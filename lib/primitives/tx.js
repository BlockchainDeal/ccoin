/*!
 * tx.js - transaction object for ccoin
 ...
 * @property {Number} version - Transaction version. Note that Ccoin reads
 ...
 * @see https://github.com/creativecoin/bips/blob/master/bip-0037.mediawiki
 * @param {Bloom} filter
...
TX.prototype.format = function format(view, entry, index) {
  ...
  return {
    ...
    value: Amount.crea(this.getOutputValue()),
    fee: Amount.crea(fee),
    rate: Amount.crea(rate),
    minFee: Amount.crea(this.getMinFee()),
    height: height,
    block: block,
    time: time,
    date: date,
 ...
 * will be reversed to abide by creativecoind's legacy
