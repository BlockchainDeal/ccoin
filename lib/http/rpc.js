...
RPC.prototype.getInfo = async function getInfo(args, help) {
  ...
  return {
    ...
    keypoolsize: 0,
    unlocked_until: 0,
    paytxfee: Amount.crea(this.network.feeRate, true),
    relayfee: Amount.crea(this.network.minRelay, true),
    errors: ''
  };
};
...
RPC.prototype.getMempoolInfo = async function getMempoolInfo(args, help) {
  ...
  return {
    ...
    mempoolminfee: Amount.crea(this.mempool.options.minRelay, true)
  };
};
...
RPC.prototype.getTXOut = async function getTXOut(args, help) {
  ...
  return {
    ...
    value: Amount.crea(coin.value, true),
...
RPC.prototype.verifyTXOutProof = async function verifyTXOutProof(args, help) {
  ...
  if (!block.verify(this.chain.network))
    return [];
...
RPC.prototype.getTXOutSetInfo = async function getTXOutSetInfo(args, help) {
  ...
  return {
    ...
    bytes_serialized: 0,
    hash_serialized: 0,
    total_amount: Amount.crea(this.chain.db.state.value, true)
  };
};
...
RPC.prototype._createTemplate = async function _createTemplate(maxVersion, coinbase, rules) {
  ...
  // See:
  // creativecoin/creativecoin#9fc7f0bce94f1cea0239b1543227f22a3f3b9274
  if (attempt.witness) {
...
RPC.prototype.validateAddress = async function validateAddress(args, help) {
  if (help || args.length !== 1)
    throw new RPCError(errs.MISC_ERROR, 'validateaddress "creativecoinaddress"');
...
RPC.prototype.txToJSON = function txToJSON(tx, entry) {
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    vout.push({
      value: Amount.crea(output.value, true),
      n: i,
...
RPC.prototype.entryToJSON = function entryToJSON(entry) {
  return {
    size: entry.size,
    fee: Amount.crea(entry.deltaFee, true),
    modifiedfee: 0,
    time: entry.time,
    height: entry.height,
