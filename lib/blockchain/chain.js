/*!
 * chain.js - blockchain management for ccoin
...
Chain.prototype.verify = async function verify(block, prev, flags) {
  ...
  if (block.prevBlock !== prev.hash) {
    console.error(block.hash('hex'), 'invalid', 'bad-prevblk', 0);
  ...
  if (!this.verifyCheckpoint(prev, hash)) {
    console.error(block.hash('hex'), 'checkpoint', 'checkpoint mismatch', 0);
  ...
  if (flags & common.flags.VERIFY_BODY) {
    ...
    if (!valid) {
      console.error(block.hash('hex'), 'invalid', reason, score);
      throw new VerifyError(block, 'invalid', reason, score, true);
    }
  }

  let blockHeight = prev.height + 1;
  if (this.network.uncheckedPowBlocks.indexOf(blockHeight) < 0) {
    // Ensure the POW is what we expect.
    const bits = await this.getTarget(block.time, prev);

    if (block.bits !== bits) {
      console.error('Block', prev.height +1, 'invalid', 'bad-diffbits', block.bits.toString(16), bits.toString(16));
      throw new VerifyError(block,
        'invalid',
        'bad-diffbits',
        100);
  ...
  if (block.time <= mtp) {
    console.error(block.hash('hex'), 'invalid', 'time-too-old', 0);
  ...
  if (block.time > this.network.now() + 2 * 60 * 60) {
    console.error(block.hash('hex'), 'invalid', 'time-too-new', 0);
  ...
  if (block.version < 2 && height >= this.network.block.bip34height) {
    console.error(block.hash('hex'), 'obsolete', 'bad-version', 0);
  ...
  if (state.hasBIP91() || state.hasBIP148()) {
    ...
    if (!consensus.hasBit(block.version, segwit.bit)) {
      console.error(block.hash('hex'), 'invalid', 'bad-no-segwit', 0);
  ...
  for (const tx of block.txs) {
    if (!tx.isFinal(height, time)) {
      console.error(block.hash('hex'), 'invalid', 'bad-txns-nonfinal', 10);
      throw new VerifyError(block,
        'invalid',
        'bad-txns-nonfinal',
        10);
  ...
  if (state.hasBIP34()) {
    if (block.getCoinbaseHeight() !== height) {
      console.error(block.hash('hex'), 'invalid', 'bad-bad-cb-height', 100);
      throw new VerifyError(block,
        'invalid',
        'bad-cb-height',
        100);
  ...
  if (state.hasWitness()) {
    ...
    if (commit) {
      ...
      if (!block.getWitnessNonce()) {
        console.error(block.hash('hex'), 'invalid', 'bad-witness-nonce-size', 100);
        throw new VerifyError(block,
          'invalid',
      ...
      if (!commit.equals(block.createCommitmentHash())) {
        console.error(block.hash('hex'), 'invalid', 'bad-witness-merkle-match', 100);
        throw new VerifyError(block,
          'invalid',
  ...
  if (!commit) {
    if (block.hasWitness()) {
      console.error(block.hash('hex'), 'invalid', 'unexpected-witness', 100);
      throw new VerifyError(block,
        'invalid',
        'unexpected-witness',
        100,
  ...
  if (block.getWeight() > consensus.MAX_BLOCK_WEIGHT) {
    console.error(block.hash('hex'), 'invalid', 'bad-blk-weight', 100);
    throw new VerifyError(block,
      'invalid',
      'bad-blk-weight',
      100);
...
Chain.prototype.getDeployments = async function getDeployments(time, prev) {
  ...
  // For some reason creativecoind has p2sh in the
  ...
  if (this.options.bip148 && this.network === Network.main) {
    if (witness !== thresholdStates.LOCKED_IN
      ...
      // since ccoin only operates on the sane
...
Chain.prototype._add = async function _add(block, flags, id) {
  ...
  if (flags & common.flags.VERIFY_POW) {
    if (!block.verifyPOW(this.network))
 ...
 * @see https://creativecointalk.org/index.php?topic=67738.0
 * @param {TX} tx
 * @returns {Promise} - Returns Boolean.
 */
...
/**
 * Check if the block must be checked with keccak PoW
 * @param height
 * @return {boolean}
 */
Chain.prototype.isChangePowActive = function (height) {
  return height >= this.network.block.changePowHeight;
};
...
Chain.prototype.getTarget = async function getTarget(time, prev) {
  ...
  const currHeight = prev.height + 1;

  let pow = null;
  if (this.isChangePowActive(currHeight)) {
    pow = this.network.keccakPow;
  } else {
  ...
  // Back 2 weeks
  let height = prev.height - (pow.retargetInterval - 1);

  if (this.isChangePowActive(prev.height+1)) {
    height = prev.height - pow.retargetInterval;
  }
...
Chain.prototype.retarget = function retarget(prev, first) {
  const currHeight = prev.height + 1;
  let isPowChange = this.isChangePowActive(currHeight);

/*  console.log(' ');
  console.log('Verifying Diff for height', prev, first);
  console.log(' ');*/
  let pow = null;
  if (isPowChange) {
    pow = this.network.keccakPow;
  ...
  let powLimit = pow.limit.clone();

  let actualTimespan = prev.time - first.time;
  if (isPowChange) {
    //console.log('retarget with keccak', pow);
    //DigiShield implementation

    if (actualTimespan < (targetTimespan - (targetTimespan / 4))) {
      actualTimespan = (targetTimespan - (targetTimespan / 4));
    }

    if (actualTimespan > (targetTimespan + (targetTimespan / 2))) {
      actualTimespan = (targetTimespan + (targetTimespan / 2));
    }

    //Restarting the difficulty with the new PoW as if it were the genesis block
    if (!this.isChangePowActive(prev.height)) {
      target = powLimit;
    }

    target = target.imuln(actualTimespan);
    target = target.idivn(targetTimespan);
    ...
    //console.log('retarget with scrypt', pow);
    ...
    let shift = target.bitLength() > 235;

    if (shift) {
      target = target.shrn(1)
    }
...
/**
 * Find a locator. Analagous to creativecoind's `FindForkInGlobalIndex()`.
 ...
 * @see https://github.com/creativecoin/bips/blob/master/bip-0009.mediawiki
