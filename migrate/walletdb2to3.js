...
function accountFromRaw(data, dbkey) {
  ...
  account.network = ccoin.network.fromMagic(p.readU32());
  ...
  account.accountKey = ccoin.hd.fromRaw(p.readBytes(82));
  account.keys = [];
  ...
  for (let i = 0; i < count; i++) {
    const key = ccoin.hd.fromRaw(p.readBytes(82));
...
function walletFromRaw(data) {
  ...
  wallet.network = ccoin.network.fromMagic(p.readU32());
...
function keyFromRaw(data, network) {
  ...
  ring.network = ccoin.network.get(network);
  ...
  if (key.length === 32) {
    ring.privateKey = key;
    ring.publicKey = ccoin.secp256k1.publicKeyCreate(key, true);
  } else {
    ring.publicKey = key;
  }
  ...
  if (script.length > 0)
    ring.script = ccoin.script.fromRaw(script);
