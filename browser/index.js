...
logger = new ccoin.logger({ level: 'debug', console: true });
logger.writeConsole = function(level, module, args) {
  var name = ccoin.logger.levelsByVal[level];
...
send.onsubmit = function(ev) {
  ...
  options = {
    outputs: [{
      address: address,
      value: ccoin.amount.value(value)
    }]
  };
...
function addItem(item, entry) {
  ...
  chainState.innerHTML = ''
    ...
    + ' value=' + ccoin.amount.crea(node.chain.db.state.value);
}
...
function formatWallet(wallet) {
  ...
  wallet.getBalance().then(function(balance) {
    html += 'Confirmed Balance: <b>'
      + ccoin.amount.crea(balance.confirmed)
      + '</b><br>';

    html += 'Unconfirmed Balance: <b>'
      + ccoin.amount.crea(balance.unconfirmed)
      + '</b><br>';
...
node = new ccoin.fullnode({
  ...
  logConsole: true,
  workers: true,
  workerFile: '/ccoin-worker.js',
  logger: logger
});

wdb = node.use(ccoin.wallet.plugin);
